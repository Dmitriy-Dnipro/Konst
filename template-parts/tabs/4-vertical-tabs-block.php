<?php
$tabs_with_top_text = get_field('tabs_with_top_text');
?>

<div class="row align-items-center tabs-with-top-text">
	<div class="title-block col-xl-4 col-lg-4 col-md-4 col-12">

		<div class="title-bl about-block__tit-padd">
			<h2 class="title-bl__title"><?= (!empty($tabs_with_top_text['title'])) ? $tabs_with_top_text['title'] : ''; ?></h2>
			<div class="title-bl__description"><?= (!empty($tabs_with_top_text['description'])) ? $tabs_with_top_text['description'] : ''; ?></div>
		</div>

	</div>
	<div class="content-block col-xl-8 col-lg-8 col-md-8 col-12">

		<div class="text"><?= (!empty($tabs_with_top_text['top_text'])) ? $tabs_with_top_text['top_text'] : ''; ?></div>

		<?php if (!empty($tabs_with_top_text['tabs'])) { ?>
			<div id="tabs" class="row">
				<ul class="col-lg-3 col-md-4 col-xs-4 col-4">
					<?php $i = 0; ?>
					<?php foreach ($tabs_with_top_text['tabs'] as $item) { ?>
						<li><a href="#tab-<?= $i+1; ?>">
							...
							<div class="tabs_bg__content"><?= (($i+1) <= 9) ? '0'.($i+1) : ($i+1); ?></div>
						</a></li>
						<?php $i++;?>
					<?php } ?>
				...
				<div class="tabs_content col-lg-9 col-md-8 col-xs-8 col-8">
					<?php $i = 0; ?>
					<?php foreach ($tabs_with_top_text['tabs'] as $item) { ?>
						<div id="tab-<?= $i+1; ?>">
							...
							<?php if (!empty($item['file'])) {?>
								<div class="file">
									<a class="main-form__inp-submit main-form__inp-submit_p file_button" href="<?= $item['file']['link']; ?>" target="_blank">
										<span>Download</span> 
										<span>
